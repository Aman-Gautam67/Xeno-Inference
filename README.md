...
```mermaid
graph TB
    subgraph Core["XENO INFERENCE CORE"]
        Router["[Zap] Intelligent Dynamic Router (Cost / Latency / Privacy)"]
        LocalEngine["[Cpu] Local Inference Engine (llama.cpp / vLLM / MLX / TensorRT-LLM)"]
        CloudGateway["[Cloud] Cloud Provider Gateway (Claude 3.7 / GPT-4o / o1 / Gemini 2.0 / DeepSeek R1)"]
        Harness["[Brain] Autonomous Agent Harness & Swarm Orchestrator"]
        LiveDAG["[Activity] Real-Time Execution DAG & Tool Graph"]
        ThinkingEngine["[Clock] Deep Thinking Timeline & Branch Inspector"]
        SpatialCanvas["[LayoutGrid] Infinite Multimodal Spatial Canvas"]
        TerminalBridge["[Terminal] Sandboxed PTY Autonomous Terminal"]
        MediaSuite["[Film] Multimodal Gen Suite (FLUX / CogVideo / Kokoro / Whisper)"]
    end

    Input(["User Input (GUI Canvas / CLI TUI / API / Voice)"]) --> Router
    Router --> LocalEngine
    Router --> CloudGateway
    LocalEngine --> Harness
    CloudGateway --> Harness
    Harness <--> LiveDAG
    Harness <--> ThinkingEngine
    Harness <--> SpatialCanvas
    Harness <--> TerminalBridge
    Harness <--> MediaSuite
```
...
```mermaid
graph LR
    UserPrompt["[User] User: Implement Payment Webhook"] --> Orchestrator["[Crown] Commander Agent"]
    
    Orchestrator --> Subagent1["[Box] Architect Agent<br/>(Spec & Schema Design)"]
    Orchestrator --> Subagent2["[Code] Coder Agent<br/>(Stripe Webhook Handler)"]
    
    Subagent1 --> Tool_FS["[Folder] filesystem_read<br/>(schema.prisma)"]
    Subagent2 --> Tool_LSP["[Search] lsp_definition<br/>(PaymentService)"]
    Subagent2 --> Tool_Write["[Edit3] file_write<br/>(webhook.ts)"]
    
    Tool_Write --> Subagent3["[FlaskConical] QA Tester Agent<br/>(Automated Unit Tests)"]
    Subagent3 --> Tool_Term["[Terminal] terminal_exec<br/>(npm test webhook)"]
    
    Tool_Term --> ResultNode{"Test Outcome"}
    ResultNode -- "[XCircle] Fail (401 Bad Sig)" --> Subagent4["[Stethoscope] Self-Healing Debugger"]
    Subagent4 --> Tool_Fix["[Bandage] multi_replace_file<br/>(webhook.ts)"]
    Tool_Fix --> Subagent3
    ResultNode -- "[CheckCircle2] Pass (200 OK)" --> FinalArtifact["[PartyPopper] Verified Output & PR"]

    style UserPrompt fill:#1e1e2e,stroke:#89b4fa,stroke-width:2px;
    style Orchestrator fill:#313244,stroke:#f38ba8,stroke-width:2px;
    style Subagent1 fill:#181825,stroke:#a6e3a1,stroke-width:2px;
    style Subagent2 fill:#181825,stroke:#a6e3a1,stroke-width:2px;
    style Subagent3 fill:#181825,stroke:#fab387,stroke-width:2px;
    style Subagent4 fill:#45475a,stroke:#eba0ac,stroke-width:2px;
    style FinalArtifact fill:#11111b,stroke:#a6e3a1,stroke-width:3px;
```
