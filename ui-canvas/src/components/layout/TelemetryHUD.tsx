...
import { Zap, Coins, HardDrive, Gauge, Clock } from "lucide-react";

export const TelemetryHUD: React.FC = () => {
  const { telemetry } = useWorkspaceStore();

  return (
    <div className="fixed top-18 right-6 z-30 p-3.5 rounded-2xl border border-border-700 bg-surface-900/85 backdrop-blur-2xl shadow-2xl flex items-center space-x-6 text-xs font-mono">
      {/* Speed Meter */}
      ...
      <div className="w-[1px] h-8 bg-border-700" />
      ...
      <div className="flex items-center space-x-2.5">
        ...
        <div>
          <div className="text-neutral-500 uppercase text-[9px] tracking-wider">TTFT</div>
          <div className="text-blue-400 font-bold text-sm tracking-tight">
            {telemetry.ttftMs} <span className="text-[10px] font-normal text-neutral-400">ms</span>
          </div>
        </div>
      </div>
